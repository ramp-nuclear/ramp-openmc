...
def openmc_lattice(lattice: CartesianLattice, transform: Transform, path: PurePath,
                   surface_cache: SurfaceCache = None, library_path=None) -> openmc.RectLattice:
    """
    ...
    default_cell = openmc_component(component=default_component, surface_cache=surface_cache,
    ...
    if isinstance(lattice.geometry, Rectangle):
        lat.lower_left = np.round(
            -lattice.dimensions * np.array(lattice.shape) / 2 + transform.translation.flatten()[:2], 4)
        shape = lattice.shape[::-1]
    else:

        lat.lower_left = np.round(-np.hstack([lattice.dimensions, [lattice.height]]) * np.hstack(
            [(np.array(lattice.shape) / 2), [1 / 2]]) + transform.translation.flatten(), 4)
        shape = np.hstack([[1], lattice.shape[::-1]])
    lat.universes = np.tile(openmc.Universe(cells=[default_cell]), shape)
    lat.outer = openmc.Universe(
        cells=[openmc_component(Component(lattice.mixture, lattice.geometry), surface_cache=surface_cache,
                                library_path=library_path)])
    return lat
...
def place_rod_in_lattice(rod: Element, grid: Grid, site: str, cells: dict,
    ...
    for l_index, l in enumerate(grid.lattices):
        if l == lattice:
            break
...
def add_boundary_condition(surfaces: Sequence[openmc.Surface],
...
def construct_box_region(lower_left: np.array,
    ...
    """
    if (len(lower_left) < 3 or lower_left[2] == -np.inf) and (
            len(upper_right) < 3 or upper_right[2] == np.inf):
        geo = Rectangle(tuple(lower_left[:2] / 2 + upper_right[:2] / 2), upper_right[:2] - lower_left[:2])
    else:
...
def _default_source_func(model):
    """
    The default source is uniform in the x and y directions and is bounded between
    -1 and 1 in the z direction. It is not taken to be uniform on the core
    as it takes too much time to create an initial sources like that.
    """
    lower_left = np.min(np.vstack(
        [lattice.outer.bounding_box[0] for lattice in model.geometry.get_all_lattices().values()]), axis=0)
    upper_right = np.max(np.vstack(
        [lattice.outer.bounding_box[1] for lattice in model.geometry.get_all_lattices().values()]), axis=0)
...
def openmc_core_to_model(core: Core, boundary_condition: str,
    ...
    cells = {}
    openmc_lattices = [openmc_lattice(lattice, transform, path, surface_cache, library_path) for
                       path, transform, lattice in
                       core.lattices()]
    ...
    for element_name, element in core.free_elements:
        universe, element_cells = openmc_universe_from_element(element, element_name, surface_cache,
                                                               library_path)
        if element_cells:
            ...
            cell = openmc.Cell(fill=universe,
                           region=construct_box_region(lower_left,
                                                       upper_right,surface_cache))
    ...
    for site, rod in core.grid.items():
        place_rod_in_lattice(rod, core.grid, site, cells, openmc_lattices,
                             surface_cache, library_path=library_path)
    for mc_lattice in openmc_lattices:
        region = construct_box_region(
            mc_lattice.lower_left,
            mc_lattice.lower_left + mc_lattice.shape * mc_lattice.pitch, surface_cache=surface_cache)
        exo_surfaces_ids.update(region.get_surfaces())
        cell = openmc.Cell(fill=mc_lattice,
                           region=region)
