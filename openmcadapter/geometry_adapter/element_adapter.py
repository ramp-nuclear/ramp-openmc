...
def openmc_universe_from_element(element: Element, element_name: str, surface_cache: SurfaceCache = None,
                                 library_path=None) -> Tuple[
    openmc.Universe, Dict]:
    """
    ...
    cells = [
        ...
        for path, comp in
        element.named_components()]
    cells_id = {PurePath(element_name) / path: cell.id for (path, _), cell in
                zip(element.named_components(), cells)}
