...
@lru_cache(maxsize=None)
def openmc_halfspace(surface: Sphere, id: int = None) -> openmc.Halfspace:  # noqa
    """
    creates an openmc halfspace form a RAMP sphere
    """
    x0, y0, z0 = surface.center
    sphere = openmc.Sphere(x0=x0, y0=y0, z0=z0, r=surface.radius, surface_id=id)
    if surface.inside:
        return -sphere
    return +sphere
...
@lru_cache(maxsize=None)
def openmc_halfspace(surface: Cylinder, id: int = None) -> openmc.Halfspace:  # noqa
    """
    creates an openmc halfspace form a RAMP cylinder
    """
    cylinder = openmc.Cylinder(
        surface.center[0],
        surface.center[1],
        surface.center[2],
        surface.radius,
...
def openmc_halfspace(surface, id=None):  # noqa
    raise NotImplementedError("We don't implement making a half space for this type of surface: {type(surface)}")
