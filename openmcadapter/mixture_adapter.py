...
_isomer_symbols = {0: "", 1: "_m1"}

_sab_symbols = {
    Chemical.LightWater: "c_H_in_H2O",
    Chemical.HeavyWater: "c_D_in_D2O",
    Chemical.Be: "c_Be",
    Chemical.Be_in_BeO: "c_Be_in_BeO",
    Chemical.Graphite: "c_Graphite",
    Chemical.O_in_BeO: "c_O_in_BeO",
    Chemical.Polyethylene: "c_H_in_CH2",
    Chemical.Benzene: "c_C6H6",
}
...
def add_burning_isotopes(mixture: Mixture, amount=1e-20):
    ...
    """
    mix = Mixture.expand(mixture)
    if U235 in mix.isotopes:
        for iso in uranium_products:
            if iso not in mix.isotopes:
                mixture.isotopes[iso] = amount


def _is_elem(nuclide: ZAID) -> TypeGuard[Isotope]:
...
def openmc_material(mixture: Mixture, add_sab=True, library_path: Path = None) -> openmc.Material:
    ...
    density = 0
    elements = list(filter(lambda n: _is_elem(n) and (f"{openmc_name(n)}0" not in available_data), mixture.keys()))
    ...
    for nuclide, nuclide_density in mixture.items():
        ...
        if nuclide_name in available_data:
            material.add_nuclide(nuclide_name, nuclide_density)
            density += nuclide_density
        elif f"{nuclide_name}0" in available_data:
            material.add_nuclide(f"{nuclide_name}0", nuclide_density)
            density += nuclide_density
        else:
            logger.debug(f"{nuclide} doesn't have data in the library so it is removed from the mixture")
    material.set_density("atom/b-cm", density)
    keys = frozenset(mixture.isotopes.keys())
    if add_sab:
        for s_ab in mixture.sab:
            if frozenset(s_ab.isotopes).intersection(keys):
