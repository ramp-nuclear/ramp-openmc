...
from openmcadapter.tally_adapter.burnup_tallies import openmc_query_tally, \
    get_result_from_statepoint
...
def _verify_extra_settings(extra_settings: dict[str, Any]):
    ...
    if len(coinciding_attributes) > 0:
        raise ValueError(f'The attributes: {", ".join(coinciding_attributes)} '
                         f'have repeated definitions.')
...
class Settings:
    ...
    def __post_init__(self) -> None:
        if self.passive_cycles > self.cycles:
            ...
            raise ValueError(f'number of {passive_cycles=} is larger'
                             f' than number of total {cycles=}')
...
class OpenMCOracle:
    ...
    def __init__(self,
        ...
        self.direct = partial(
            ...
            directory=d, boundary_condition=boundary_condition)
    ...
    threads_factory - function that determines the number of threads.
    ...
    def __call__(self, state: OperationalState,
                 *queries: Query,
                 ...
                 prefix: str = '', **kwargs) -> OracleResult:
        """
        ...
        return self.direct(state, *queries, tasks=tasks, prefix=prefix,
                           boundary_condition=boundary_condition,
                           extra_tallies=extra_tallies,
                           settings=replace(self.settings, **kwargs))


def _model(core: Core,
    ...
    """
    model, cells_ids, surface_cache = openmc_core_to_model(core, boundary_condition,
                                                           temperature_method=settings.temperature_method,
                                                           library_path=settings.library_xml_path)
    if extra_tallies:
        model, tallies = extra_tallies(model=model, cells=cells_ids)
    ...
    for query in queries:
        if isinstance(query, KQuery):
            ...
            model.settings.surf_source_write = dict(surface_ids=surface_ids,
                                                    max_particles=query.maximal_number_of_particles or 1000)
            surface_file_location = query.path
        elif isinstance(query, SurfaceCurrentQuery):
            t = surface_current_tally(query, cells_ids, dict(core.named_components), model.geometry,
                                      surface_cache)
            tallies[query] = t.id
            q_tallies[query] = t.id
            model.tallies.append(t)
...
def _load_tallies(workspace: Path, sp: Optional[openmc.StatePoint] = None,
    ...
    tallies_by_query = {query: sp.tallies[tally_id] for query, tally_id in
                        q_tallies.items()}
    ...
    for query in queries:
        results[query] = get_result_from_statepoint(tallies_by_query,
                                                    cells_ids, query,
                                                    named_components)
    return results


def run_openmc(state: OperationalState,
               *queries: Query,
               settings: Settings,
               tasks: Optional[int] = None,
    ...
    model, tallies, q_tallies, cells_ids, extra_keys, surface_file_location = _model(core, queries, settings,
                                                                                     boundary_condition, extra_tallies)
    with TemporaryDirectory(prefix=prefix, suffix='_openmc', dir=directory,
                            clean_dir=cleanup) as tmpdir:
        workspace: Path = Path(tmpdir)
        model.export_to_xml(directory=workspace)
