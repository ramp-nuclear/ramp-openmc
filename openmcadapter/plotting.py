...
def _get_limits(origin: tuple[cm, cm, cm], width: tuple[cm, cm], basis: BASIS) -> tuple[tuple[cm, cm], tuple[cm, cm]]:
    match basis:
        case "xy":
            x, y = 0, 1
        case "yz":
        ...
        case "xz":
            ...
            x, y = 0, 2
        case _:
            raise ValueError(f"The basis was not xy, yz or xz: {basis}")
...
def _plot_model(
    ...
    resolution: cm = None,
    origin: tuple[cm, cm, cm] = (0.0, 0.0, 0.0),
    path: Path = None,
    basis: BASIS = "xy",
    colors: dict[openmc.Cell, COLOR_SPECIFIER] = None,
    complement_color: COLOR_SPECIFIER = None,
    aspect="equal",
    ax: plt.Axes = None,
    **kwargs,
) -> mpl.image.AxesImage | None:
    resolution = resolution or min(appropriate_resolution(DEFAULT_PIXELS_NUM, size) for size in width)
    plot = openmc.Plot()
    plot.width = width
    plot.pixels = tuple(appropriate_pixels(resolution, size) for size in width)
    plot.origin = origin
    plot.basis = basis
    if colors:
        if complement_color:
            cells = model.geometry.get_all_cells().values()
            for cell in filter(lambda x: x not in colors.keys(), cells):
    ...
    with TemporaryDirectory(prefix="openmc_plotter") as tempdir:
        if path:
            ...
            if ax is None:
                ...
                fig.set_dpi(dpi)
            (x_min, x_max), (y_min, y_max) = _get_limits(origin, width, basis)
            return ax.imshow(img, extent=(x_min, x_max, y_min, y_max), interpolation="none", aspect=aspect, **kwargs)
...
def plot_core(
    ...
    grid_color: COLOR_SPECIFIER = None,
    aspect="equal",
    ax: plt.Axes = None,
    sites_map=False,
    **kwargs,
    ...
    if colors:
        if all(isinstance(key, Mixture) for key in colors.keys()):
            colors = {path: colors[component.mixture] for path, component in core.named_components}
        colors = {cells[cell_ids[path]]: colors[path] for (path, node) in core.named_components}
    ...
    ax = _plot_model(model, width, resolution, origin, path, basis, colors, grid_color, aspect=aspect, ax=ax, **kwargs)
    if sites_map:
...
def plot_element(
    ...
    basis: BASIS = "xy",
    colors: MIXTURE_MAPPING | PATH_MAPPING = None,
    aspect: Literal["equal", "auto"] | float = "equal",
    ax: plt.Axes = None,
    **kwargs,
) -> mpl.image.AxesImage | None:
    f"""
    ...
    universe, cell_ids = openmc_universe_from_element(deepcopy(element), element_name)
    if colors:
        if all(isinstance(key, Mixture) for key in colors.keys()):
            colors = {path: colors[component.mixture] for path, component in element.named_components()}
        colors = {
            universe.cells[cell_ids[element_path / path]]: colors[path]
            for (path, node) in element.named_components()
            if element_path / path in cell_ids
        }
