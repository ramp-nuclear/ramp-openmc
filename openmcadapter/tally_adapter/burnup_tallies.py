...
scores_dict = {Typus.NFission: "fission",
...
openmc_particles = {Neutron: 'neutron',
                    Photon: 'photon',
                    Electron: 'electron',
                    Proton: 'proton',
		    Positron: 'positron'}
...
def openmc_particle(particle: NamedParticle):
    try:
        return openmc_particles[particle]
    except KeyError:
        raise ValueError(f"{particle} calculations are not supported by "
...
def openmc_query_tally(query: VolumeQuery, cells_ids: dict[PurePath, int]) \
    ...
    tally.nuclides = list(
        set(openmc_name(score.reaction.parent)
            for score in query.scores if isinstance(score, ReactionScore)))
...
def gather_score_from_tally(tally: openmc.Tally,
                            score: str,
                            cell_ids: tuple[int],
                            particle: str,
    ...
    filter_bins = [cell_ids,
                   (particle,)]
    if energy_bins != ((0., np.inf),):
        filters.append(openmc.EnergyFilter)
        filter_bins.append(energy_bins)
    shape = (len(cell_ids), len(energy_bins))
    mean_vals = tally.get_values(scores, filters,
                                 filter_bins, nuclides, 'mean').reshape(shape)
    error_vals = tally.get_values(scores, filters, filter_bins,
                                  nuclides, 'std_dev').reshape(shape)
...
def result_dict(score: Score, path: PurePath,
                energy_bin: tuple[eV, eV],
                particle: NamedParticle,
                mean: float, error: float) -> dict:
    data = {'component': path,
            'score': score,
            'value': mean,
            'error': error,
            'particle': particle.name.lower()}
...
def get_score_results(tally: openmc.Tally,
                      score: Score,
                      query: VolumeQuery,
                      cell_ids: dict[PurePath, int],
                      named_components: dict[PurePath, Component]) -> Iterable[dict]:
    cell_ids = tuple(cell_ids[path] for path in query.names)
    nuclides = [openmc_name(score.reaction.parent)] \
        if isinstance(score, ReactionScore) else []
    ...
    if score.volume_specific:
        division_factors *= np.fromiter((named_components[path].geometry.volume
                                         for path in query.names), dtype=float)
    if isinstance(score, ReactionScore):
        if score.density_specific:
            division_factors *= np.fromiter((named_components[path].mixture[score.reaction.parent]
                                             for path in query.names),
                                            dtype=float)
    division_factors = division_factors if (division_factors != 1).any() else None
    mean, error = gather_score_from_tally(tally, openmc_score(score),
                                          cell_ids, openmc_particle(particle),
                                          energy_bins, nuclides,
                                          division_factors)
    for (i, path), (j, energy_bin) in product(enumerate(query.names),
                                              enumerate(energy_bins)):
        yield result_dict(score, path, energy_bin, particle,
                          mean=mean[i, j],
                          error=error[i, j])
