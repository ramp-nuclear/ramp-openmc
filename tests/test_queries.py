...
@pytest.fixture(scope="module")
def uranium_paths():
    return tuple(PurePath(f"{site}/coolant/aluminum_block/uranium_block") for site in example_state.core.grid.keys())


@pytest.fixture(scope="module")
def scores():
    return (
        Score("flux", volume_specific=False),
        ReactionScore(
...
def queries(uranium_paths, scores):
    return {
        "k": KQuery(),
        "entire_energy_flux": VolumeQuery(names=uranium_paths, scores=scores[:1]),
        "binned_energy_flux": VolumeQuery(names=uranium_paths, energies=(0.0, 2.0, 30.0, 30e6), scores=scores[:1]),
        "entire_energy_reaction": VolumeQuery(names=uranium_paths, scores=scores[1:]),
        "binned_energy_reaction": VolumeQuery(names=uranium_paths, energies=(0.0, 2.0, 30.0, 30e6), scores=scores[1:]),
    }
...
def test_results_are_sensible(results, uranium_paths, queries, scores, ndarrays_regression):
    ...
    for path in uranium_paths:
        for name, score in zip(["flux", "reaction"], scores):
            entire_energy_result = first(
                result["value"]
                for result in results[queries[f"entire_energy_{name}"]]
                if result["score"] == score and result["component"] == path
            )
            split_energy_result = sum(
                [
                    result["value"]
    ...
    tallies_results = {
        attr: np.array(list(map(itemgetter(attr), results[queries[f"binned_energy_{name}"]])), dtype=typ)
        for attr, typ in zip(
            ["component", "value", "error", "particle", "lower_energy", "upper_energy"],
...
def test_surface_currents_from_opposite_symmetric_sides_are_close_by_example(direct):
    ...
    query3 = SurfaceCurrentQuery(
        surface_down,
        from_component=PurePath("CoreTree/pool/south_hafnium_block"),
        to_component=PurePath("CoreTree/pool"),
    )
    result = direct(example_state, query1, query2, query3)
    assert np.abs(abs(result[query1][0]["value"]) - abs(result[query2][0]["value"])) < 2 * result[query1][0]["error"]
...
def test_surface_currents_equal_mesh_surface_currents_by_example(direct):
    ...
    def mesh_tally(model, *_, **__):
        ...
        mesh.lower_left = bottom_hafnium_block_center - hafnium_block_dimensions / 2
        mesh.upper_right = bottom_hafnium_block_center + hafnium_block_dimensions / 2
...
def test_cartesian_mesh_volume_division(direct):
    ...
    integral_query = MeshQuery(mesh, scores=(integral_flux_score,), energies=(0.0, 10.0, 20.0e6))
    average_query = MeshQuery(mesh, scores=(average_flux_score,), energies=(0.0, 10.0, 20.0e6))
    ...
    assert np.allclose(
        (integral_result["mean"] / average_result["mean"]).to_series()[lambda x: x.notna()], d**3, equal_nan=True
    )
    assert np.allclose(
        (integral_result["std"] / average_result["std"]).to_series()[lambda x: x.notna()], d**3, equal_nan=True
    )
...
def test_compare_tabulated_heating_to_heating_tally_of_water():
    ...
    """
    heating_neutron_query = VolumeQuery(
        scores=tuple([Score("heating", volume_specific=True)]), names=tuple([PurePath("CoreTree/pool")])
    )
    heating_photon_query = VolumeQuery(
        scores=tuple([Score("heating", volume_specific=True)]),
    ...
    neutron_tabulated_queries = valmap(
        lambda tab: VolumeQuery(
            names=tuple([PurePath("CoreTree/pool")]), scores=(TabulatedScore(tuple(tab.x), tuple(tab.y), True),)
        ),
        neutron_tabulated,
    )
    ...
    photon_tabulated_queries = valmap(
        lambda tab: VolumeQuery(
            names=tuple([PurePath("CoreTree/pool")]),
            scores=(TabulatedScore(tuple(tab.x), tuple(tab.y), True),),
            particle=Photon,
        ),
        photon_tabulated,
    )
